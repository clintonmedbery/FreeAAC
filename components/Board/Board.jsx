...
const Board = (props) => {
  ...
  const speechTags = speechItems.map((boardItem, index) => {
    return (
      <View key={uuidv4()}>
        {boardItem.imageUrl && (boardItem.imageUrl.endsWith('svg') ?
          <SvgUri
            style={styles.image}
            width="80%"
            height="80%"
  ...
  return (
      <Grid>
        ...
        <Row style={{color: "black", height: 50}}>
          {speechTags}
          <Button danger rounded style={{
            marginLeft: 'auto', marginRight: 10, marginTop: 5
          }}
          onPress={() => {
            console.log("Cliicckkkk");
            setSpeechItems([])
          }}
          >
            <Icon name={"close"}/>
